...
require 'phpmailer/vendor/autoload.php';
 ...
 $email_err = $last_name_err = $message_err = $subject_err ="";
 $email = $last_name = $message =$subject = $success = $fail = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["name"])) {
    ...
    $last_name = test_input($_POST["name"]);
  ...
  if (empty($_POST["subject"])) {
    $subject_err = "<span class='error'>Telegram/Skype name Required!</span>";
  } else {
  ...
  if (empty($_POST["email"])) {
   ...
   if ( $last_name_err == "" and $email_err == "" and $subject_err == "" and $message_err == ""){
...
$mail->addAddress('onukwube.dewey208@gmail.com',$last_name);
...
$mail->Body='<h1 align=center>Name:'.$last_name.'<br>
Telegram/Skype Name'.$subject.'<br>Email:'.$email.'
...
if (!$mail->send()) {
    ...
    $success = 'Email not sent! If problem persist reach us on telegram!'; 
    echo"<script>window.open('index.php#contact','_self')</script>";
} else {

    $fail = 'Message has sent,thank you '.$last_name.' for contacting luxequity.io! ';
    echo"<script>window.open('index.php#contact','_self')</script>";
}
